    ...
    return(
        <div className="w-full h-full bg-bg-surface p-2 flex flex-row gap-5 items-center justify-between">
            <div className="w-1/3 h-full flex flex-row gap-3 justify-center items-start self-start ">
                <Button value={get_repo_name()} width={9.5} height={3} fontSize={12} onClick={() => handleRepoClick()} className="min-w-7 bg-transparent hover:bg-border tracking-widest self-center" />
                <DropDown childs={branches.map((branch, i) => ({ key: i, value: branch }))} label={branches[Number(selected)]} width={7} height={3} fontSize={12} selected={selected} setSelected={setSelect} className="bg-transparent hover:bg-border tracking-widest self-center" />
                <div className="h-full w-full flex flex-row gap-5 items-center justify-center">
